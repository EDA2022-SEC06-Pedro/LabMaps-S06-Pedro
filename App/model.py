...
def newTitle(name):
    ...
    title = {'name': "",
              "books": None}
    title['name'] = name
    title['books'] = lt.newList('SINGLE_LINKED', compareTitles )
    return title
...
def addBook(catalog, book):
    ...
    title = book['title']
    addBookTitle(catalog, title, book)
...
def compareTitles(title, book):
    ...
    """
    titlentry = me.getKey(book)
    if (title == titlentry):
        return 0
    elif (title > titlentry):
        return 1
    else:
        return -1
